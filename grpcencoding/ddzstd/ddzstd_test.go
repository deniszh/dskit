...
func TestDDZstd(t *testing.T) {
	c := newCompressor()
	assert.Equal(t, "ddzstd", c.Name())
...
func BenchmarkDDZstdCompress(b *testing.B) {
...
func BenchmarkDDZstdDecompress(b *testing.B) {
	data := []byte(strings.Repeat("123456789", 1024))
	c := newCompressor()
	var buf bytes.Buffer
	w, _ := c.Compress(&buf)
...
func BenchmarkDDZstdGrpcCompressionPerf(b *testing.B) {
	data := []byte(strings.Repeat("123456789", 1024))
	...
	for i := 0; i < b.N; i++ {
		for j := 0; j < 10; j++ {
			var buf bytes.Buffer
			writer, _ := c.Compress(&buf)
			writer.Write(data)
			writer.Close()

			compressedData := buf.Bytes()
			reader, _ := c.Decompress(bytes.NewReader(compressedData))
